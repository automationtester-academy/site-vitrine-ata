  ...
  return (
    <>
      <nav className="navbar flex items-center justify-between flex-wrap bg-slate-900 p-4 lg:px-36 py-2 border-b-2 border-white max-xl:!pl-[4%]">
        <Link
          to="/"
          className="navbar-logo flex items-center flex-shrink-0 text-white"
          onClick={closeMenu}
        >
          <ATALogo />
        </Link>
        ...
        <div
          className={`NavbarLinks w-full block flex-grow lg:flex lg:items-center lg:w-auto ${
            isMenuOpen ? "" : "hidden"
          } lg:block`}
        >
          <div className="NavbarLinks_menu text-sm lg:flex-grow lg:flex lg:justify-center lg:ml-36 mt-3 lg:mt-0">
            <Link
              to="/ecole"
              className="block mt-4 text-lg lg:inline-block lg:mt-0 text-white hover:text-cyan-600 hover:underline mr-4 font-semibold max-xl:text-md"
              onClick={closeMenu}
            >
              LՎcole
            </Link>
          ...
          <div className="NavBarLinks_button flex-shrink-0 my-2.5 lg:my-0">
            <div className="relative inline-block p-[1.2px] pl-[1.75px] bg-gradient-to-r from-blue-500 to-cyan-500 rounded-md">
              <Link
                to="/contact"
                data-cy="contact-us"
                className="Header-text block px-5 text-md py-2.5 leading-none bg-slate-900 text-white rounded-md hover:text-white hover:bg-cyan-600 font-semibold max-xl:text-md"
                onClick={closeMenu}
              >
                Nous Contacter
              </Link>
